...
static ssize_t attr_show(const struct class *class, const struct class_attribute *attr,char *buf){
	return sprintf(buf,"%s\n",KBUILD_MODNAME);
}
static ssize_t attr_store(const struct class *class, const struct class_attribute *attr,const char *buf, size_t count){
	pr_info("input: %s",buf);
	return count;
}
CLASS_ATTR_RW(attr);
...
static int __init charu_init(void){
	...
	if(class_create_file(sys,&class_attr_attr)){
		pr_err("class_create_file failed");
		goto file;
	}
...
static void __exit charu_exit(void){
	pr_info("removed file from sysfs");
	class_remove_file(sys,&class_attr_attr);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("A way with sysfs");
